...
public class InventarioController {
    ...
    public ResponseEntity<Boolean> verificarStock(
        ...
        return ResponseEntity.ok(inventarioService.verificarStock(idProducto, cantidad));
    }

    // Endpoint para Pedidos: ¡Descuenta el stock!
    @PutMapping("/descontar/{idProducto}")
    public ResponseEntity<Void> descontarStock(
            @PathVariable Long idProducto, 
            @RequestParam Integer cantidad) {
        
        inventarioService.descontarStock(idProducto, cantidad);
